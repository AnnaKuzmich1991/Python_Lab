...
class TestHouse(unittest.TestCase):
    ...
    def setUp(self):
        self.house = House(1, 50, 2, 2, "Lenina", "Brick", 1999)

    def test_set_num_flat_correct(self):
        self.assertEqual(self.house.get_num_flat(), 1)
        self.house.set_num_flat(2)
        self.assertEqual(self.house.get_num_flat(), 2)

    def test_get_num_flat(self):
        self.assertEqual(self.house.get_num_flat(), 1)
...
class House:
    ...
    def set_num_flat(self,
                     num_flat):  # метод объекта set_num_flat, который устанавливает новое значение для инкапсулированного поля __num_flat
        if num_flat < 1:
    ...
    def calculate_age(
            lifetime):  # статический метод, который принимает дату постройки здания и вычисляет возраст здания на текущий год, который закодирован в методе (2023)
